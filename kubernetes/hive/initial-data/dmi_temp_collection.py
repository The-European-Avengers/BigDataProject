#!/usr/bin/env python3
"""
DMI Temperature Collection
Collects temperature data (mean_temp) from all Danish weather stations that have temperature measurements.
...
def get_mean_temp_stations(year: int) -> dict:
    """Discover all stations that have mean_temp observations for the given year,
    ...
    try:
        ...
        if not station_ids:
            raise RuntimeError("No stations found for mean_temp in selected period")
        ...
        if not filtered_ids:
            raise RuntimeError("No Denmark (DNK) stations found for mean_temp in selected period")
...
def fetch_station_month(station_id, year, month):
    """Fetch mean temperature data for one station for one month."""
...
def collect_temperature_data(year):
    """Collect mean temperature data for one year from all temperature-capable stations."""
    print(f"Collecting mean_temp data for {year}")
    stations = get_mean_temp_stations(year)
    print(f"Stations discovered with mean_temp in {year}: {len(stations)}")
    if not stations:
        raise RuntimeError("No stations discovered for mean_temp; aborting.")
    ...
    for station_id, station_name in stations.items():
        print(f"\nProcessing {station_name} ({station_id}), temperature data")
        ...
        for month in range(1, last_month + 1):
            ...
            if features:
                ...
                for feature in features:
                    ...
                    temperature_value = props.get("value")

                    # Only add records with actual temperature data
...
def save_data(data, year):
    """Save collected mean temperature data to CSV in the shared volume."""
    if not data:
    ...
    # Remove any rows where mean_temp is null (extra safety)
    original_count = len(df)
    df = df.dropna(subset=['mean_temp'])
    cleaned_count = len(df)
    
    if original_count != cleaned_count:
        print(f"Cleaned {original_count - cleaned_count} null temperature records")
    
    # Escribe el archivo en la ruta del volumen compartido
    filename = f"{SHARED_DATA_PATH}/{year}_dmi_temp.csv"
    df.to_csv(filename, index=False)
    
    print(f"\nSaved {len(df)} temperature records to shared volume path: {filename}")
    print(f"Date range: {df['timeObserved'].min()} to {df['timeObserved'].max()}")
    print(f"Stations: {df['stationId'].nunique()}")
    print(f"Average mean_temp: {df['mean_temp'].mean():.2f}")
    print(f"Max mean_temp: {df['mean_temp'].max():.2f}")
...
def main():
    ...
    for TARGET_YEAR in TARGET_YEARS:
        ...
        # Collect mean temperature data for this year
        try:
            temperature_data = collect_temperature_data(TARGET_YEAR)
        except Exception as e:
            print(f"Error during collection for {TARGET_YEAR}: {e}")
            continue

        # Save to CSV
        output_file = save_data(temperature_data, TARGET_YEAR)
